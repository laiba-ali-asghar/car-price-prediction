...
# ==========================================================
# Model Feature List (Hardcoded to bypass dynamic attribute errors)
# This list MUST match the 25 features used in your ColumnTransformer
# ==========================================================
MODEL_FEATURES = [
    'symboling', 'normalized-losses', 'wheel-base', 'length', 'width', 
    'height', 'curb-weight', 'engine-size', 'bore', 'stroke', 
    'compression-ratio', 'horsepower', 'peak-rpm', 'city-mpg', 'highway-mpg',
    'num-of-doors', 'num-of-cylinders', # These two are converted to numbers later
...
def load_pipeline_and_data():
    ...
    try:
        ...
        # Simple cleaning for EDA (drop price NaNs, convert known numerical-like columns)
        ...
        st.error(f"Error loading files. Please check scikit-learn version in requirements.txt (must match version that saved the .pkl): {e}")
        st.stop()
...
with col_input:
    ...
    # Row 3: Other Categorical Features (Passed as strings, converted to numbers in the prediction block)
    ...
    # These values are set to the median/mode the model was trained with or np.nan for imputation
    ...
    user_input['engine-type'] = df_cleaned['engine-type'].mode()[0] # Mode
    user_input['fuel-system'] = df_cleaned['fuel-system'].mode()[0] # Mode
...
with col_output:
    ...
    if st.button("Predict Price", type="primary"):
        try:
            ...
            st.error(f"Prediction Error: An unexpected error occurred during prediction. Please re-check your inputs. Details: {e}")
    else:
